...
@click.option(
    "--regenerate-catnap",
    is_flag=True,
    help="Regenerate CATNAP references from FASTA files",
    default=False
)
@click.option(
    "--regenerate-igl",
    is_flag=True,
    help="Regenerate IGL references from FASTA files",
    default=False
)
@click.option(
    "--skip-blast",
    is_flag=True,
    help="Skip BLAST database generation",
    default=False
)
@click.option(
    "--skip-aux",
    is_flag=True,
    help="Skip auxiliary file generation",
    default=False
)
@click.option(
    "--skip-internal",
    is_flag=True,
    help="Skip internal annotation file generation", 
    default=False
)
@click.option(
    "-v",
...
def make_all(
    ...
    1. Downloads IMGT germline sequences (if needed)
    2. Generates IgBLAST reference databases
    3. Creates auxiliary files for IgBLAST
    4. Generates internal annotation files (ndm.imgt)
    5. Optionally regenerates CATNAP and IGL references
    6. Creates reference index files
    ...
    # Step 1: Load reference configuration
    click.echo("\n" + "="*60)
    click.echo("STEP 1: Loading reference configuration")
    click.echo("="*60)
    reference_object = References.from_yaml(reference)
    ...
    # Step 2: Regenerate CATNAP references if requested
    if regenerate_catnap:
        click.echo("\n" + "="*60)
        click.echo("STEP 2: Regenerating CATNAP references")
        click.echo("="*60)
        from pathlib import Path
        import subprocess
    ...
    # Step 3: Regenerate IGL references if requested
    if regenerate_igl:
        click.echo("\n" + "="*60)
        click.echo("STEP 3: Regenerating IGL references")
        click.echo("="*60)
    ...
    # Step 4: Generate all database files
    click.echo("\n" + "="*60)
    click.echo("STEP 4: Generating database files")
    click.echo("="*60)
    ...
    click.echo("\n" + "="*60)
    click.echo("STEP 5: Verifying generated files")
    click.echo("="*60)
