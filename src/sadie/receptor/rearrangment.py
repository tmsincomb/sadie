...
class ReceptorChain(BaseModel):
    ...
    def from_single(
        ...
        airr_table = airr.run_single(
            sequence_id=sequence_id, sequence=sequence, reference_name=reference_name, database=database
        )
        if airr_table is None:
            raise ValueError("could not annotate the sequence")
        return airr_table.to_receptor_chain()
