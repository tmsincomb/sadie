...
- Q: Auto-Gapping Implementation Method - What tool/approach should be used for IMGT gapping of ungapped sequences? → A: Use existing Sadie ANARCI/HMMER alignment capabilities
- Q: G3 Migration Strategy & Timeline - Should G3 be removed immediately or phased out gradually? → A: Parallel operation with feature flag (G3 as fallback), then deprecation after validation period
...
**Independent Test**: User can add a FASTA file to `sources/custom/human/IGHV.fasta`, run Sadie annotation, and see the custom allele used in results. Delivers immediate value for researchers with novel alleles.

**Acceptance Scenarios**:

1. **Given** user has a novel IGHV sequence, **When** they add it to `sources/custom/human/IGHV.fasta` and run Sadie, **Then** the pipeline auto-detects the change, rebuilds databases, and annotates sequences using the custom allele
...
1. **Given** IMGT and OGRDB data populated in sources/, **When** user runs Sadie annotation without internet, **Then** annotation completes successfully using local data
2. **Given** sources/ directories are empty, **When** user attempts annotation, **Then** system provides clear error with instructions to populate data (references download scripts)
...
1. **Given** VDJbase FASTA files in sources/vdjbase/human/, **When** manager loads genes, **Then** VDJbase sequences are parsed and available
...
1. **Given** empty sources/ directories, **When** user runs `scripts/download_imgt.py human`, **Then** IMGT FASTA files are downloaded and validated with INFO-level logging showing progress
...
- **What happens when sources/ files change mid-annotation?** Pipeline checks hashes at start; if files change during run, current run completes with old data, next run triggers rebuild
...
- **What if two providers have sequences that are 99% similar but different names?** Both included (not considered duplicates); only exact sequence matches trigger deduplication
- **How to handle mixed gapped/ungapped sources?** Auto-detect gaps (contains "." or "-"); convert ungapped to gapped using existing Sadie ANARCI/HMMER alignment for V/J segments
...
- **FR-003**: VDJbaseProvider MUST handle manually-added FASTA files in sources/vdjbase/ directory
...
#### Data Population
- **FR-005**: System MUST provide download scripts for IMGT data (`scripts/download_imgt.py`)
- **FR-006**: System MUST provide download scripts for OGRDB data (`scripts/download_ogrdb.py`)
- **FR-007**: System MUST document manual download instructions for VDJbase data in `sources/vdjbase/README.md` (automated download script is optional future enhancement)
...
- **FR-009b**: Expected file structure validation MUST verify: (1) species subdirectory exists (e.g., sources/imgt/human/), (2) segment files exist (IGHV.fasta, IGHD.fasta, IGHJ.fasta minimum for heavy chain), (3) file permissions are readable
...
- **FR-010**: System MUST document manual download instructions in sources/*/README.md for each provider
...
- **FR-017**: System MUST support parallel operation mode where germlines module is primary and G3 acts as fallback for validation period
- **FR-017a**: Parallel operation mode is DEPRECATED - validation period will use feature flag toggle (FR-016) instead of automatic fallback to reduce complexity
- **FR-017b**: Validation period duration MUST be: minimum 30 days of production use OR 100% test pass rate on 3 consecutive releases, whichever comes first
- **FR-017c**: Validation period success criteria: (1) SC-004 maintained (100% test pass), (2) zero critical bugs filed against germlines module, (3) performance metrics within 10% of G3 baseline
...
- **FR-021**: System MUST gap V and J segment sequences using IMGT numbering scheme via existing Sadie ANARCI/HMMER alignment capabilities
- **FR-021a**: ANARCI integration interface MUST call existing Sadie method: `sadie.renumbering.anarci.number_sequence(sequence, scheme="imgt", chain_type="H")` returning (numbered_sequence, start, end)
- **FR-021b**: Gapper MUST translate nucleotide sequence to amino acid before calling ANARCI, then map gaps back to nucleotide sequence using codon positions
- **FR-021c**: If ANARCI numbering fails (returns None or raises exception), system MUST log WARNING "Failed to gap sequence {gene_name}: {error}" and store ungapped version only
...
- **FR-022a**: Normalized directory structure MUST be: normalized/{species}/{segment}_{chain}_gapped.fasta and normalized/{species}/{segment}_{chain}_ungapped.fasta
- **FR-023**: D segments MUST be stored ungapped only (no gapping required)
- **FR-024**: Gapping implementation MUST reuse existing Sadie renumbering infrastructure to avoid duplication
...
- **FR-027a**: Mock data structure MUST include: sources/mock/{provider}/{species}/{segment}.fasta with 2-3 genes per segment for fast CI/CD execution
...
- **FR-029**: System MUST include curated test dataset (5-10 genes per segment) checked into repository under `tests/data/germlines/`
...
- **FR-031**: Test data MUST include edge cases: invalid sequences, duplicate names, identical sequences with different names
...
- **FR-037b**: CDR/FWR boundaries MUST be determined using IMGT numbering scheme via ANARCI alignment: FWR1(1-26), CDR1(27-38), FWR2(39-55), CDR2(56-65), FWR3(66-104)
...
- **FR-038a**: BLAST database files MUST be stored in igblast/{species}/ with naming convention: {species}_{segment}.ndb, .nsq, .nin, .nhr
- **FR-039**: System MUST generate IgBLAST internal_data files: organism.yaml containing species metadata and available segments
- **FR-039a**: organism.yaml format MUST be: `{species}: {chain: {segments: [V, D, J], path: "igblast/{species}/"}}`
...
- **FR-036b**: Common failure mode error messages MUST include: (1) Missing data: "IMGT data not found at {path}. Cause: sources/ directory empty. Remediation: Run scripts/download_imgt.py human", (2) Invalid FASTA: "Invalid sequence in {file}:{line}. Cause: Contains non-ACGT characters. Remediation: Check sequence contains only ACGT", (3) BLAST build fail: "makeblastdb failed with exit code {code}. Cause: {stderr}. Remediation: Check disk space and file permissions"

### Key Entities

- **VDJbaseProvider**: Provider implementation for VDJbase database. Inherits from GermlineProvider. Handles VDJbase-specific FASTA parsing and metadata extraction. Reads manually-added FASTA files from sources/vdjbase/ directory. API automation is deferred as optional future enhancement.
...
- **Test Dataset**: Curated collection of germline sequences (5-10 genes per segment) stored in `tests/data/germlines/`. Includes representative examples from all providers with both gapped/ungapped variants. Covers edge cases for comprehensive unit testing without requiring full reference data download.
...
- **SC-009**: Change detection correctly triggers rebuild only when sources/ files change (verified via timestamp checks)
...
6. **Migration strategy**: Phased migration approach with feature flag for controlled rollout. Germlines module is primary (default), G3 maintained as fallback during validation period. Validation period duration: minimum 30 days of production use OR 100% test pass rate on 3 consecutive releases, whichever comes first. Success criteria: (1) SC-004 maintained (100% test pass), (2) zero critical bugs, (3) performance within 10% of G3 baseline. After successful validation and 60-day deprecation notice (CHANGELOG + runtime WARNING + GitHub discussion), G3 dependencies removed completely. **Note**: FR-017 parallel operation mode (automatic fallback) is deprecated in favor of explicit feature flag toggle (FR-016) to reduce complexity.
