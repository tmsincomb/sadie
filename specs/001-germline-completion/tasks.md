...
- [ ] T003 [P] Create test data directory structure in tests/data/germlines/ (custom/, imgt/, ogrdb/, vdjbase/)
- [ ] T004 [P] Create curated test dataset with 5-10 genes per segment in tests/data/germlines/
...
- [ ] T009 [P] Implement auto-gapping service using ANARCI in src/sadie/germlines/builders/gapper.py
...
**Independent Test**: User adds FASTA to `sources/custom/human/IGHV.fasta`, runs Sadie, sees custom allele in results
...
- [ ] T020 [P] [US1] Write unit test for custom sequence priority in tests/test_custom_provider.py
- [ ] T021 [P] [US1] Write integration test for custom sequence end-to-end in tests/test_integration.py
...
**Acceptance Criteria**:
- [ ] User can add novel IGHV to sources/custom/human/IGHV.fasta
...
**Acceptance Criteria**:
- [ ] User runs `python scripts/download_imgt.py human` and gets validated FASTA files
...
- [ ] T039 [P] [US2] Write offline integration test (network disabled) in tests/test_offline_operation.py
- [ ] T040 [US2] Verify cached data usage (6 months old) works correctly in src/sadie/germlines/pipeline.py

**Acceptance Criteria**:
- [ ] IMGT and OGRDB data populated in sources/
- [ ] Sadie annotation completes without internet access
- [ ] Empty sources/ gives clear error with remediation steps
...
- [ ] T044 [US4] Implement G3 API response format adapter in src/sadie/reference/reference.py
...
- [ ] T050 [P] [US4] Test feature flag SADIE_USE_GERMLINES_MODULE=false (G3 fallback)
...
- [ ] T053 [P] [US3] Write unit test for priority ordering scenarios in tests/test_priority_ordering.py
- [ ] T054 [P] [US3] Test deduplication rules (same name, same sequence, novel) in tests/test_priority_ordering.py
...
- [ ] T059 [P] [US5] Write unit tests for VDJbase provider in tests/test_vdjbase_provider.py
- [ ] T060 [P] [US5] Create VDJbase test data in tests/data/germlines/vdjbase/
- [ ] T061 [US5] Test VDJbase in priority ordering in tests/test_priority_ordering.py
...
**Acceptance Criteria**:
- [ ] VDJbase FASTA files in sources/vdjbase/human/ are parsed successfully
...
**Test Data**: Curated dataset in `tests/data/germlines/` with 5-10 genes per segment covering edge cases
...
- [ ] ANARCI integration: T009 implements gapper with fallback to ungapped on failure
- [ ] VDJbase format changes: T062 adds error handling with clear remediation
- [ ] G3 parity: T044 adapter maintains format, T050 tests fallback
...
1. Initial: SADIE_USE_GERMLINES_MODULE=true (default), G3 available as fallback
...
**Generated**: 2026-01-08
**Total Duration Estimate**: 20-25 hours
